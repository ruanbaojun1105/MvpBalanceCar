...
//        IMNotificationUtils.getInstance().showToast(fragment.getActivity(), "用户点击了url:" + url);
        ...
        WebviewActivity.newInstance(fragment.getContext(),url);
...
//        fragment.startActivity(intent);
    ...
    public static void sendGeoMessage(YWConversation conversation) {
        conversation.getMessageSender().sendMessage(
                YWMessageChannel.createGeoMessage(MainActivity.location.getLatitude(),
                        MainActivity.location.getLongitude(), MainActivity.location.getAddrStr()), 120, null);
    }
...
//            IMNotificationUtils.getInstance().showToast(fragment.getActivity(), "你点击了地理位置消息");
            ...
            LatLng lng = new LatLng(message1.getLatitude(), message1.getLongitude());
            MapLeidaActivity.newInstance(fragment.getContext(),lng);
            return true;
...
//            }
            ...
            final YWConversation conversation =LoginHelper.getInstance().getIMKit().getConversationService().getConversationByConversationId(message.getConversationId());
...
//            IMNotificationUtils.getInstance().showToast(context, "你长按了自定义消息");
    ...
    /**
     * 获取url对应的商品详情信息，当openIM发送或者接收到url消息时会首先调用{ChattingOperationCustomSample#getCustomUrlView(Fragment, YWMessage, String, YWConversation)},
     * 若getCustomUrlView()返回null，才会回调调用该方法获取商品详情，若getCustomUrlView()返回非null的view对象，则直接用此view展示url消息，不再回调该方法。因此，如果希望该方法被调用,
     * 请确保{ChattingOperationCustomSample#getCustomUrlView(Fragment, YWMessage, String, YWConversation)}返回null。
     * @param fragment  可以通过 fragment.getActivity拿到Context
...
//            GoodsInfo info = new GoodsInfo("商品", "60.03", "86.25", "8.00", bitmap);
//            return info;
//        }
        return null;
    }
...
//                });
        ...
        if (conversation.getConversationType() == YWConversationType.P2P&& MainActivity.location!=null) {
...
//            replyBarItem3.setItemImageRes(R.drawable.im_reply_bar_private_image);
...
//            replyBarItems.add(replyBarItem3);
            ...
            replyBarItem.setItemLabel("你好\t\to(*￣︶￣*)o");
            replyBarItem.setOnClicklistener(new View.OnClickListener(){
                @Override
                public void onClick(View v) {
